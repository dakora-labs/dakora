...
3. Development Mode (if `AUTH_ENABLED=false`)

## Authentication Modes

### Production Mode (Auth Enabled)

When `AUTH_REQUIRED=true` (or `AUTH_ENABLED=true` on backend):
...
- Verify `AUTH_ENABLED` is `true` in backend configuration
...
# Without auth (should fail if AUTH_ENABLED=true)
...
<AccordionGroup>
  ...
  <Accordion title="API requests return 401 Unauthorized">
    ...
    **Solutions:**
    ...
    - Ensure `AUTH_ENABLED=true` in backend config
