---
title: API Keys
description: 'Configure API keys for LLM providers and Dakora authentication'
---
...
2. **Dakora API Keys** - For authenticating programmatic access to Dakora APIs (see [Authentication Guide](/guides/authentication))

## Dakora API Keys

For authentication and API access, see the [Authentication Guide](/guides/authentication#api-key-authentication).
...
Response fields include `id`, `name`, `key` (only on creation), `key_prefix`, `created_at`, `expires_at`, and masked `key_preview` values for reads.
