...
from dakora_server.core.llm.client import LLMClient
from dakora_server.core.exceptions import APIKeyError, RateLimitError, ModelNotFoundError, LLMError
...
def test_execute_success(llm_client, mock_litellm_response):
    with patch("dakora_server.core.llm.client.completion", return_value=mock_litellm_response):
...
def test_execute_anthropic(llm_client):
    ...
    with patch("dakora_server.core.llm.client.completion", return_value=mock_response):
...
def test_execute_authentication_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=AuthenticationError("Invalid API key", llm_provider="openai", model="gpt-5")):
...
def test_execute_rate_limit_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=LiteLLMRateLimitError("Rate limit exceeded", llm_provider="openai", model="gpt-5")):
...
def test_execute_model_not_found_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=BadRequestError("model not found", llm_provider="openai", model="invalid-model")):
...
def test_execute_bad_request_non_model_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=BadRequestError("Invalid parameter", llm_provider="openai", model="gpt-5")):
        with pytest.raises(LLMError) as exc_info:
            llm_client.execute("Test prompt", "gpt-5")
        assert "Bad request" in str(exc_info.value)

def test_execute_timeout_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=Timeout(message="Request timed out", model="gpt-5", llm_provider="openai")):
        with pytest.raises(LLMError) as exc_info:
            llm_client.execute("Test prompt", "gpt-5")
...
def test_execute_api_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=APIError(status_code=500, message="API error occurred", llm_provider="openai", model="gpt-5")):
        with pytest.raises(LLMError) as exc_info:
            llm_client.execute("Test prompt", "gpt-5")
        assert "API error" in str(exc_info.value)

def test_execute_unexpected_error(llm_client):
    with patch("dakora_server.core.llm.client.completion", side_effect=Exception("Unexpected error")):
        with pytest.raises(LLMError) as exc_info:
            llm_client.execute("Test prompt", "gpt-5")
...
def test_execute_with_temperature_param(llm_client, mock_litellm_response):
    with patch("dakora_server.core.llm.client.completion", return_value=mock_litellm_response) as mock_completion:
