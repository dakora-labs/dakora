...
def cleanup_provisioning_tests(request, db_connection):
    """Clean up test data created by provisioning tests."""
    # Skip for unit tests
    if hasattr(request, 'cls') and request.cls and 'TestHelperFunctions' in request.cls.__name__:
        yield
        return
...
class TestProvisioningIntegration:
    ...
    def test_provision_workspace_and_project(self, db_connection):
        ...
        workspace_id, project_id = provision_workspace_and_project(
            db_connection, user_id, "Bogdan Pshonyak", "test-bogdan@example.com"
        )
        db_connection.commit()
        ...
        assert workspace.slug.startswith("bogdan")  # May be bogdan-1, bogdan-2, etc. due to session fixtures
    ...
    def test_provision_workspace_unique_slug(self, db_connection):
        """Test provisioning handles duplicate slugs."""
        # Create two test users with same name
        user1_result = db_connection.execute(
            users_table.insert().values(
        ...
        user2_result = db_connection.execute(
            users_table.insert().values(
        ...
        workspace1_id, _ = provision_workspace_and_project(
            db_connection, user1_id, "John Doe", "test-john1@example.com"
        )
        db_connection.commit()

        # Provision second workspace (should get unique slug)
        workspace2_id, _ = provision_workspace_and_project(
            db_connection, user2_id, "John Doe", "test-john2@example.com"
        )
        db_connection.commit()

        # Verify both workspaces have unique slugs
        workspace1 = db_connection.execute(
            select(workspaces_table).where(workspaces_table.c.id == workspace1_id)
        ).fetchone()
        workspace2 = db_connection.execute(
            select(workspaces_table).where(workspaces_table.c.id == workspace2_id)
        ).fetchone()

        # Both should start with "john" and second should have a numeric suffix
        assert workspace1.slug.startswith("john")
        assert workspace2.slug.startswith("john")
        assert workspace1.slug != workspace2.slug  # Must be unique
        # Second workspace should have numeric suffix (e.g., john-1, john-2, etc.)
        assert "-" in workspace2.slug and workspace2.slug.split("-")[1].isdigit()

    def test_provision_workspace_first_name_only(self, db_connection):
        ...
        # Provision workspace and project
        workspace_id, project_id = provision_workspace_and_project(
            db_connection, user_id, "Alice", "test-alice@example.com"
        )
        db_connection.commit()
    ...
    def test_get_or_create_workspace_creates_new(self, db_connection, db_engine):
        ...
        workspace_id, project_id = get_or_create_workspace(
            db_engine, user_id, "Lazy User", "test-lazy1@example.com"
        )
    ...
    def test_get_or_create_workspace_returns_existing(self, db_connection, db_engine):
        ...
        original_workspace_id, original_project_id = provision_workspace_and_project(
            db_connection, user_id, "Existing User", "test-existing@example.com"
        )
        db_connection.commit()

        # Call get_or_create_workspace (should return existing)
        workspace_id, project_id = get_or_create_workspace(
            db_engine, user_id, "Existing User", "test-existing@example.com"
        )
    ...
    def test_get_or_create_workspace_handles_race_condition(self, db_connection, db_engine):
        ...
        workspace_id_1, project_id_1 = get_or_create_workspace(
            db_engine, user_id, "Race User", "test-race@example.com"
        )
